...
array<array<uint8_t,NP>,NM>P;array<array<uint64_t,6>,NP>Z;
...
int main(int ac,char**av){if(ac<5){cerr<<"moves paths pid source_limit\n";return 2;}int pid=stoi(av[3]),lim=stoi(av[4]);ifstream mf(av[1]);for(int m=0;m<NM;m++)for(int i=0;i<NP;i++){int x;mf>>x;P[m][i]=x;}for(int i=0;i<NP;i++)for(int c=0;c<6;c++)Z[i][c]=sm(1234567+i*911+c*131);
 ifstream pf(av[2]);string ln;vector<vector<int>>paths;while(getline(pf,ln)){istringstream ss(ln);vector<int>w;int x;while(ss>>x)w.push_back(x);paths.push_back(w);}auto path=paths[pid];int L=path.size();State v0;for(int i=0;i<NP;i++)v0[i]=i/25;vector<State>A(L+1);A[0]=v0;for(int i=0;i<L;i++)A[i+1]=mv(A[i],path[i]);
